...
\]
as a 'show trend criteria'.

\subsubsection{Marginal show trend probability}
\label{sec:marginal-show-trend}

Then marginally, we have the show trend probability
\[
  \begin{aligned}
...
\subsubsection{Show trend probability conditional on global significance (Z-statistics)}
\label{sec:show-trend-prob-z-stat}

The Z-statistics version is actually \textbf{assuming $\sigma^2$ is known}. Hence the global test is constructed as to reject $H_0$ when
\[
  \frac{\left|\bar{x} - \bar{y}\right|}{
    \sqrt{\left(\frac{1}{n_1} + \frac{1}{n_2}\right)\sigma^2}
  }
  = \frac{\left|\bar{x} - \bar{y}\right|}{
    \sqrt{\frac{1 + k}{n_1} \cdot \sigma^2}
  }
  > z_{1 - \alpha / 2}
\]
where we follow the assumption that $n_1 = k n_2$. Therefore the show trend probability given global significance is given by
\[
  \begin{aligned}
    & P\left(
    \Delta_r \geq \rho\Delta
    \middle|
    \left|\Delta\right| > z_{1 - \alpha / 2}
    \sqrt{\frac{1 + k}{n_1} \cdot \sigma^2}
    \right)    \\
    =& P\left(
       \Delta_r \geq \rho\Delta
       \middle|
       \left\{
       \Delta > z_{1 - \alpha / 2}\sqrt{\frac{1 + k}{n_1} \cdot \sigma^2}
       \right\}
       \cup
       \left\{
       \Delta < z_{\alpha / 2}\sqrt{\frac{1 + k}{n_1} \cdot \sigma^2}
       \right\}
       \right)
  \end{aligned}
\]
...
\subsubsection{Show trend probability conditional on global significance (T-statistic)}
\label{sec:show-trend-prob-t-stat}

From Section~\ref{sec:two-sample-variance-1}, we know the test is constructed as to reject $H_0$ when
\[
  \frac{
    \left|\bar{x} - \bar{y}\right|}{
    \sqrt{
