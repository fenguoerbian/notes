...
\[
  ...
  .
\]
Unfortunately, both $\lambda$ and $p$ (essentially $G\left(\cdot\right)$) are unknown. We plug in the estimation $\hat{\lambda} = r / W$ and $\hat{p} = r / n$ and apply Slutsky's theorem. This means for the purpose of estimation, we use
\begin{equation}
  ...
  \right.
\end{equation}
Note that unlike \eqref{eq:fisher_info_property}, here in \eqref{eq:fisher_info_estimate}, $r$ and $W$ are observations. And we have
\begin{equation}
...
\]
\par
\citet{Breslow1974p-} show that as $n\to\infty$,
\[
  \sqrt{n}\left(
  ...
  ,
\]
which can be easily proved by if we express the KM esimator as a martingale process (the original proof in \citet{Breslow1974p-} is quite lengthy).
\par
I think this previous work is summarised in \citet{Brookmeyer1982p29-29}, that we can consider a transform function $g$, such that $g\left(S\left(t\right)\right)$ follows normal distribution. Again from $\delta$-method, we know that
\[
...
({\color{blue} I think this is the idea from \citet{Brookmeyer1982p29-29}.}) The KM estimator can be used to estimated survival time and construct corresponding CI at given survival probability. For example, the median survival time $t_m$ can be estimated as $\hat{t}_m$, the solution such as
\[
