...
\[
  ...
  .
\]
Here in \eqref{eq:discrete_likelihood} we use the approximation that $e^{-h_j} \approx 1 - h_j$ when $h_j$ is close to 0. Then we can use the method of \underline{profile likelihood}: That is, for any given $\beta$, we maximize $L$ (or equivalently, $\mathrm{log}L$) over $h_j$s so the result is a function of $\beta$. Taking derivative, we have
\[
