...
\par
ETI and HDI can produce same results as long as the distribution is symmetric. But in reality many posterior distribution is skewed, then HDI is a more reasonable CI. A demonstration is shown in Figure~\Ref{fig:eti_and_hdi}.
\begin{figure}[htbp]
  \centering
...
Note that if want to make some prediction/statement about $x$, like $x\in A$, then the probability can be computed as
\begin{equation}
